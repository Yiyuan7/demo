...
var expectedElbowPose;
var expectedNosePose;
...
async function loadSampleImg() {
  ...
  image.src = `https://cdn.shopify.com/s/files/1/2367/6873/products/product-image-391219229_1024x1024@2x.jpg?v=1505607451`;
  return promise;
}
...
async function decodeSinglePoseAndDrawResults() {
  ...
  console.log(pose);
  expectedNosePose = pose.keypoints[0].position;
  expectedElbowPose = pose.keypoints[7].position;
  console.log(expectedNosePose);
  console.log(expectedElbowPose);

  writeFileSync('pose.txt' , pose);
...
async function testImageAndEstimatePoses(net) {
  ...
  // testImage = await loadImageFromPath('./xxx.jpg');
//  testImage = await loadSampleImg();

  image = document.getElementById('xxx');
